...
MSG_MAIN_MENU="
	...
	===================
	A. Add the selection box
	R. Re install to remove the selection box
	===================
...
showMainMenu() {
	...
	case $flag in
		...
		6)
			...
			;;
		A)
			echo "add selection box success "
			div='<span>Server: </span><select style="width:120px;height:25px;border-radius:4px;border-color:#95B8E7;margin-right:8px;" onchange="window.location=this.value"><option>请选择</option>'
			opl='<option value="'
			rk='">'
			opr='</option>'
			selr='</select>'
			...
			function change(){
    				...
    				sed -i "s/<span>Theme: <\/span>/$end/g" /usr/share/transmission/web/index.html
			}
			while_read
			change
			;;
